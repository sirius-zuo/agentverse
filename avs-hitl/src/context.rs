...
impl HitlContext {
    pub fn new(
        ...
        Self { session_id, skill_id, policy, queue }
    }
}
...
impl HitlHook for HitlContext {
    async fn check_tool(&self, tool_name: &str, args: &Value) -> Option<(ApprovalId, String)> {
        let kind = if HitlPolicy::is_checkpoint_tool(tool_name) {
            ...
            InterruptKind::SkillCheckpoint { checkpoint_name: name, payload }
        } else if self.policy.requires_tool_approval(self.skill_id.as_deref(), tool_name) {
            InterruptKind::ToolApproval { tool_name: tool_name.to_string(), args: args.clone() }
        } else {
            return None;
        };
